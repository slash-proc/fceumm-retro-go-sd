...
def load_mapper_dict(repo_root: Path) -> dict[int, str]:
    """Import mapper_dict from src/fceumm/gen_mappers_table.py without
    running its CLI tail (which sys.exit()s when called with no arguments)."""
    p = repo_root / "src/fceumm/gen_mappers_table.py"
    if not p.is_file():
        # Legacy layout used by the firmware monolith tree.
